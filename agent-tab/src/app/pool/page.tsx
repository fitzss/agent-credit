  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        ...
        <h1 className="text-3xl font-bold mt-2">Pool</h1>
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <VersionBadge version={r.contractVersion} />
            </div>
            <button
              onClick={() => refreshReserve(r.id)}
              disabled={refreshing}
          ...
          <div className="grid grid-cols-3 gap-4 text-sm">
            ...
            <div>
              ...
              <p className="font-mono text-zinc-400">
                {r.reserveTokenId.substring(0, 16)}...
              </p>
            </div>
            <div>
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        {o.settlementReadiness === "ready" && o.reserveId && (
                          <button
                            onClick={() =>
                              redeemObligation(o.reserveId!, o.id)
                            }
...
/* ---- Inline components (matching existing page patterns) ---- */
...
function PoolStatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`inline-block px-2.5 py-1 text-sm rounded-full font-medium ${styles[status] || styles.offline}`}
    >
...
function ReadinessBadge({ readiness }: { readiness: string }) {
  ...
  return (
    <span
      className={`inline-block px-2 py-0.5 text-xs rounded-full ${styles[readiness] || styles["no-debt"]}`}
    >
...
function LifecycleBadge({ lifecycle }: { lifecycle: string }) {
  ...
  return (
    <span
      className={`inline-block px-2 py-0.5 text-xs rounded-full ${styles[lifecycle] || "bg-zinc-800 text-zinc-400"}`}
    >
      {lifecycle}
    </span>
  );
  ...
  return (
    <span
      className={`inline-block px-2 py-0.5 text-xs rounded-full ${
        isV2 ? "bg-blue-900/50 text-blue-400" : "bg-zinc-800 text-zinc-400"
      }`}
    >
      {version}
    </span>
